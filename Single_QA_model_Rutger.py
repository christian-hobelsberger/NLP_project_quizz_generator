...
save_path_model_train = os.path.join(os.environ.get('TMPDIR'), 'results_CL', 'model_CL_train')
save_path_model_test = os.path.join(os.environ.get('TMPDIR'), 'results_CL', 'model_CL_test')
...
def detokenize_func(tokenizer, token_ids, label_ids):
    ...
    parts = []
    parts.append(tokenized_support.split(tokenizer.sep_token)[0])
    for i in range(5):
