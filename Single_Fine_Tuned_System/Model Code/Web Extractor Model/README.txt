...
QA_WEB_large_model.py in this folder is the code which resulted in the trained model utilized to generate the results from 
the paper. It was run on the HPC Hábròk from the RUG, using a NVIDIA A100 GPU. It is heavily commented for reproducibility.
...
4. Press run, it will first preprocess the data and show the filtering process in the output, and then start training. 
After training, it will generate some outputs on the unseen data split, however this is not part of the evaluation of 
...
Joint Pipeline Final.py in the main folder shows how to load and use the model to generate outputs, if you have the model and
